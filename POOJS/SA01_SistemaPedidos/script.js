...
class Pedido{
    ...
    // método valor total
    calcularValorTotal() {
        let total = this.#itens.reduce((acc, item) => acc + item.produto.getPreco()*item.quantidade,0);  // reduzir 
...
class SistemaPedidos{
    ...
    cadastrarProduto() {
        const nome = document.getElementById("produtoNome");  // DOM do HTML
        const preco = document.getElementById("produtoPreco");  // DOM do HTML
        if(!nome.vlaue.trim() || preco<=0) return alert("Informe os Dados do Produto Corretamente!");
        // instanciar objeto
        const produto = new Produto(  // MODEL
            this.produtos.length+1,  // id = quantidade de produtos do vetor +1
            nome.value.trim(),
            preco.value
        )
    ...
    atualizarProduto() {
        ...
        this.produtos.forEach(produto => {
            // lista produtos
            const li = document.createElement("li");
            li.textContent = `${produto.getNome()} - R$ ${produto.getPreco()}`;  // duas casas decimais após a virgula (toFixed)
            lista.appendChild(li);
            ...
            divProdutos.appendChild(document.createElement("br"));  // br = pular linha
        });
    }
    ...
    gerarPedido() {
        ...
        if(itens.length === 0) return alert("Selecione pelo menos um Porduto!");
        // instanciar o pedido
...
// instanciar um obejto da classe sistemaPedidos
